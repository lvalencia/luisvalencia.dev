  ...
  describe("#isISO8601String", () => {
    it("indicates it's an ISO8601 string", () => {
      iso8601Strings.forEach((iso8601String) => {
        expect(isISO8601String(iso8601String)).to.be.true
      });
    });
    it("indicates it's not an ISO8601 string", () => {
      notISO8601Strings.forEach((notISO8601String) => {
        expect(isISO8601String(notISO8601String)).to.be.false;
      });
    });
  });
  describe("#toISO8601", () => {
    it("returns a string", () => {
      iso8601Strings.forEach((iso8601String) => {
        expect(toISO8601(iso8601String)).to.be.ok;
      });
    });
    it("returns nothing", () => {
      notISO8601Strings.forEach((notISO8601String) => {
        expect(toISO8601(notISO8601String)).to.not.be.ok;
      });
    });
  });
});
