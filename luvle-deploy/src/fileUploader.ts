...
interface UploadClient {
  upload(
    params: UploadParams,
    callback?: (err: Error, data: UploadResultData) => void
  ): unknown;
}
...
export interface Uploader {
  upload(
    validFiles: ValidFilePath[],
    basePath: ValidFilePath
...
export class FileUploader implements Uploader {
  ...
  constructor(args: FileUploaderArgs) {
    ...
    this.resolveContentType = fromMaybe({
      ...
      fallback: lookup as ContentTypeResolverFunction,
    });
  ...
  private uploadFile(
    ...
    this.logger.log(
      `Uploading ${file} of Content Type ${contentType} to ${this.uploadTarget}/${filePath}`
    );
