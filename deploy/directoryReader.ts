...
type ReadDirectoryFunction = (path: string, options: { encoding: BufferEncoding, withFileTypes: boolean } & any) => Dirent[];

export interface FileSystemReader {
    read(filePath: ValidFilePath, options?: { recursively?: boolean }): ValidFilePath[];
}
...
export class DirectoryReader implements FileSystemReader {
    ...
    public read(sourceFilePath: ValidFilePath, {recursively} = { recursively: true }): ValidFilePath[] {
        this.clearReadFilePaths();
        this.performReadOperation(sourceFilePath, recursively);
        return this.filePaths();
    }

    private performReadOperation(sourceFilePath: ValidFilePath, recursively = false): void {
        this.readDirectory(sourceFilePath, { encoding: 'utf-8', withFileTypes: true }).forEach((entry) => {
            const {
                name: fileName,
            } = entry;
